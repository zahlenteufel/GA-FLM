...
void copy(Chromosome& c1, Chromosome& c2) {
  copy(c1.begin(), c1.end(), c2.begin());
}

bool Crossover_Operator::decide_to_do_crossover() {
  return random_number_in_01() < crossover_probability;
}

// do crossover of pop1 and leave the result in pop2...
void Crossover_Operator::do_crossover(const vector<Chromosome>& pop1, vector<Chromosome>& pop2) {
  int population_size = pop1.size(), chromosome_length = pop1[0].size();
  for (int index = 0; index + 1 < population_size; index += 2) {
    for (int j = 0; j < chromosome_length; j++) {
      Chromosome& new1 = pop1[index], new2 = pop1[index + 1], old1 = pop2[index], old2 = pop2[index + 1];
      copy(new1, old1);
      copy(new2, old2);
...
void OnePoChromosomeCrossover::do_crossover(Chromosome& c1, Chromosome& c2) {
  int chromosome_length = c1.size();
  // swap tails
...
pair<int, int> CrossoverOperator::pick_range() {
  int i1 = pick_crosssite(), i2 = pick_crosssite();
  while (i1 == i2)
    i2 = pick_crosssite();
